...
bool SEASON3B::CNewUIWindowMenu::UpdateMouseEvent()
{
	...
	if(m_iSelectedIndex > -1 && SEASON3B::IsRelease(VK_LBUTTON))
	{
		...
		case 8:
			{
				g_pNewUISystem->Toggle(SEASON3B::INTERFACE_CUSTOM_RANKING);
				g_pNewUISystem->Hide(SEASON3B::INTERFACE_WINDOW_MENU);
				return false;
			}
			break;
		case 9:
			{
				g_pNewUISystem->Toggle(SEASON3B::INTERFACE_CUSTOM_EVENTTIME);
				g_pNewUISystem->Hide(SEASON3B::INTERFACE_WINDOW_MENU);
				return false;
			}
			break;
...
void SEASON3B::CNewUIWindowMenu::RenderTexts()
{
	...
	int iTextNumber[] = {1741, 1742, 364, 1743, 3055, 3103, 3107, 3108, 0, 0};
	static const char* szCustomText[] = { "Custom Ranking", "Event Schedule" };
	...
	for(int i=0; i<MENU_MAX_INDEX; ++i)
	{
		...
		const char* text = (i < 8) ? GlobalText[iTextNumber[i]] : szCustomText[i - 8];
		g_pRenderText->RenderText(m_Pos.x, y, text, 112, 0, RT3_SORT_CENTER);
		y += 20.f;
	}
}
