...
// --- Protocol structures ---
#pragma pack(push, 1)
struct PMSG_CUSTOM_ARENA_LIST_REQ
{
    BYTE c;
    BYTE size;
    BYTE head;
...
// (list send uses raw buffer: C1+size+F3+E9+count(4)+[PMSG_CUSTOM_ARENA_ENTRY...])

struct PMSG_CUSTOM_ARENA_JOIN_REQ
{
    BYTE c;
    BYTE size;
    BYTE head;
...
// (scoreboard send uses raw buffer: C1+size+F3+EF+arenaIndex(4)+count(4)+[PMSG_CUSTOM_ARENA_SCORE_ENTRY...])
#pragma pack(pop)
// --- End protocol structures ---
...
public:
	...
	// --- Protocol handlers ---
	void GCReqArenaList(int aIndex);
	void GCReqJoinArena(int aIndex, PMSG_CUSTOM_ARENA_JOIN_REQ* lpMsg);
	void GCSendArenaListTo(int aIndex);
	void GCSendStateUpdate(CUSTOM_ARENA_INFO* lpInfo);
	void GCSendScoreboard(CUSTOM_ARENA_INFO* lpInfo);
private:
