...
void ReceiveServerList( BYTE *ReceiveBuffer )
{
	...
	if (g_bAutoTest)
	{
		g_ConsoleDebug->Write(MCD_NORMAL, "[AutoTest] Server list received, auto-selecting first server");
		rUIMng.HideWin(&rUIMng.m_ServerSelWin);
		rUIMng.HideWin(&rUIMng.m_LoginMainWin);

		CServerGroup* pGroup = g_ServerListManager->GetServerGroupByBtnPos(0);
		if (pGroup == NULL || pGroup->GetServerSize() <= 0)
			pGroup = g_ServerListManager->GetServerGroupByBtnPos(1);
		if (pGroup && pGroup->GetServerSize() > 0)
		{
			CServerInfo* pInfo = pGroup->GetServerInfo(0);
			if (pInfo && pInfo->m_iPercent < 100)
			{
				SendRequestServerAddress(pInfo->m_iConnectIndex);
			}
		}
	}
}
        ...
        case 0x01:
            ...
            if (g_bAutoTest)
            {
                ...
                for(int i=0;i<SIZE_PROTOCOLVERSION;i++)
                {
                    ...
                    { bVersionOK = false; break; }
                }
                if (bVersionOK)
                {
                    g_ConsoleDebug->Write(MCD_NORMAL, "[AutoTest] Auto-logging in");
                    rUIMng.HideWin(&rUIMng.m_LoginWin);
                    SendRequestLogIn("111", "111");
                }
            }
            break;
...
BOOL TranslateProtocol( int HeadCode, BYTE *ReceiveBuffer, int Size, BOOL bEncrypted)
{
	...
	case 0xE5:
		...
		break;
	case 0xE6:
		if(ReceiveBuffer[0] == 0xC2 && g_pNewUICustomRanking != NULL)
		{
			g_pNewUICustomRanking->ReceiveRankingData(ReceiveBuffer);
		}
		else
		{
		...
		break;
	case 0xE7:
		if(g_pNewUICustomRanking != NULL)
		{
			if(ReceiveBuffer[0] == 0xC1)
			{
				PMSG_CUSTOM_RANKING_COUNT_SEND* pCount = (PMSG_CUSTOM_RANKING_COUNT_SEND*)ReceiveBuffer;
				g_pNewUICustomRanking->ReceiveRankingCount(pCount->count);
			}
		}
		break;
	case 0xE8:
		if(g_pNewUICustomEventTime != NULL)
		{
			g_pNewUICustomEventTime->ReceiveEventTime(ReceiveBuffer);
		}
		break;
	case 0xEA:
		if(g_pNewUICustomBuyVip != NULL)
		{
			g_pNewUICustomBuyVip->ReceiveVipInfo(ReceiveBuffer);
		}
		break;
		case 0xEE:
			if(g_pNewUICustomArena != NULL)
			{
				g_pNewUICustomArena->ReceiveArenaList(ReceiveBuffer);
			}
			break;
		case 0xEF:
			if(g_pNewUICustomArena != NULL)
			{
				g_pNewUICustomArena->ReceiveScoreboard(ReceiveBuffer);
			}
			break;
