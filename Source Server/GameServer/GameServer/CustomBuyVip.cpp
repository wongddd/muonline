...
void CCustomBuyVip::GCReqBuyVipInfo(int aIndex)
{
#if (GAMESERVER_CLIENTE_UPDATE >= 6)
	if (gObjIsConnectedGP(aIndex) == 0)
	{
	...
	BYTE send[4096];
	PMSG_CUSTOM_BUYVIP_INFO_SEND pMsg;
	...
	for (int n = 0; n < MAX_CUSTOM_BUYVIP; n++)
	{
		if (this->m_CustomBuyVipInfo[n].Index != n)
			continue;

		memcpy(&send[size], &this->m_CustomBuyVipInfo[n], sizeof(CUSTOM_BUYVIP_INFO));
		size += sizeof(CUSTOM_BUYVIP_INFO);
		pMsg.count++;
	}
