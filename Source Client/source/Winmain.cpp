...
// Auto-test mode for headless functional testing
extern void CreateLogInScene();
...
bool CreateOpenglWindow()
{
	...
	// Try to enable multisampling (MSAA)
	int iMSAAPixelFormat = -1;
	if (InitGLMultisample(g_hInst, g_hWnd, pfd, 4, iMSAAPixelFormat))
	{
		g_bSupportedMSAA = TRUE;
		g_ErrorReport.Write("> MSAA 4x supported, recreating context.\r\n");

		wglMakeCurrent(NULL, NULL);
		wglDeleteContext(g_hRC);
		SetPixelFormat(g_hDC, iMSAAPixelFormat, &pfd);
	...
	else
	{
		...
		g_ErrorReport.Write("> MSAA not supported, using standard pixel format.\r\n");
	}
...
//#ifdef _DEBUG
	...
	case 6:WindowWidth = 1600; WindowHeight = 1280; break;
	case 7:WindowWidth = 1600; WindowHeight = 900; break;
	case 8:WindowWidth = 1680; WindowHeight = 1050; break;
	                 ...
	                 _In_ LPSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	if (g_pProtect->ReadMainFile("Main.info") == 0)
	{
		MessageBox(0, "Main.info not found or invalid!", "Error", MB_OK | MB_ICONERROR);
		ExitProcess(0);
	}
	...
	// Initialize built-in GLSL shaders
	if (InitBuiltinShaders())
	{
		g_ErrorReport.Write( "> Shaders initialized.\r\n");
	}
	else
	{
		g_ErrorReport.Write( "> Shaders not available, using fixed-function only.\r\n");
	}
	g_ErrorReport.AddSeparator();
