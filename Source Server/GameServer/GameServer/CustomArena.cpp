...
void CCustomArena::GCReqArenaList(int aIndex) // OK
{
    this->GCSendArenaListTo(aIndex);
}

void CCustomArena::GCReqJoinArena(int aIndex, PMSG_CUSTOM_ARENA_JOIN_REQ* lpMsg) // OK
{
    if(OBJECT_RANGE(aIndex) == 0) return;

    int arenaIndex = lpMsg->arenaIndex;
    if(arenaIndex < 0 || arenaIndex >= MAX_CUSTOM_ARENA) return;

    CUSTOM_ARENA_INFO* lpInfo = &this->m_CustomArenaInfo[arenaIndex];

    if(lpInfo->State != CUSTOM_ARENA_STATE_STAND && lpInfo->State != CUSTOM_ARENA_STATE_START)
    {
        gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,this->m_CustomArenaText9);
        return;
    }
    ...
    if(this->GetUserCount(lpInfo) >= ((lpInfo->RuleInfo.MaxUser>MAX_CUSTOM_ARENA_USER)?MAX_CUSTOM_ARENA_USER:lpInfo->RuleInfo.MaxUser))
    {
        gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,this->m_CustomArenaText10);
        return;
    }

    if(lpInfo->RuleInfo.MinLevel != -1 && lpInfo->RuleInfo.MinLevel > lpObj->Level)
    {
        gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,this->m_CustomArenaText11,lpInfo->RuleInfo.MinLevel);
        return;
    }
    if(lpInfo->RuleInfo.MaxLevel != -1 && lpInfo->RuleInfo.MaxLevel < lpObj->Level)
    {
        gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,this->m_CustomArenaText12,lpInfo->RuleInfo.MaxLevel);
        return;
    }
    if(lpInfo->RuleInfo.MinMasterLevel != -1 && lpInfo->RuleInfo.MinMasterLevel > lpObj->MasterLevel)
    {
        gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,this->m_CustomArenaText13,lpInfo->RuleInfo.MinMasterLevel);
        return;
    }
    if(lpInfo->RuleInfo.MaxMasterLevel != -1 && lpInfo->RuleInfo.MaxMasterLevel < lpObj->MasterLevel)
    {
        gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,this->m_CustomArenaText14,lpInfo->RuleInfo.MaxMasterLevel);
        return;
    }
    if(lpInfo->RuleInfo.MinReset != -1 && lpInfo->RuleInfo.MinReset > lpObj->Reset)
    {
        gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,this->m_CustomArenaText15,lpInfo->RuleInfo.MinReset);
        return;
    }
    if(lpInfo->RuleInfo.MaxReset != -1 && lpInfo->RuleInfo.MaxReset < lpObj->Reset)
    {
        gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,this->m_CustomArenaText16,lpInfo->RuleInfo.MaxReset);
        return;
    }
    if(lpInfo->RuleInfo.MinMasterReset != -1 && lpInfo->RuleInfo.MinMasterReset > lpObj->MasterReset)
    {
        gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,this->m_CustomArenaText17,lpInfo->RuleInfo.MinMasterReset);
        return;
    }
    if(lpInfo->RuleInfo.MaxMasterReset != -1 && lpInfo->RuleInfo.MaxMasterReset < lpObj->MasterReset)
    {
        gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,this->m_CustomArenaText18,lpInfo->RuleInfo.MaxMasterReset);
        return;
    }
    if(lpInfo->RuleInfo.RequireClass[lpObj->Class] == 0 || lpInfo->RuleInfo.RequireClass[lpObj->Class] > (lpObj->ChangeUp+1))
    {
        gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,this->m_CustomArenaText19);
        return;
    }

    // Move player to arena start gate
    gObjMoveGate(aIndex, lpInfo->RuleInfo.StartGate);
}

void CCustomArena::GCSendArenaListTo(int aIndex) // OK
{
    if(OBJECT_RANGE(aIndex) == 0) return;

    BYTE send[8192];
    int count = 0;
    int ofs = 7; // after C1+size+headcode(EE)+count(4)

    for(int n=0; n<MAX_CUSTOM_ARENA; n++)
    {
        CUSTOM_ARENA_INFO* lpInfo = &this->m_CustomArenaInfo[n];
        if(lpInfo->State == CUSTOM_ARENA_STATE_BLANK) continue;

        PMSG_CUSTOM_ARENA_ENTRY* pEntry = (PMSG_CUSTOM_ARENA_ENTRY*)&send[ofs];
        pEntry->Index = lpInfo->Index;
        pEntry->State = lpInfo->State;
        pEntry->RemainTime = lpInfo->RemainTime;
        pEntry->UserCount = this->GetUserCount(lpInfo);
        pEntry->MaxUsers = lpInfo->RuleInfo.MaxUser;
        pEntry->EnterEnabled = lpInfo->EnterEnabled;
        memcpy(pEntry->Name, lpInfo->RuleInfo.Name, 32);
    ...
    *(int*)(send+3) = count;
    ...
    DataSend(aIndex, send, ofs);
}

void CCustomArena::GCSendStateUpdate(CUSTOM_ARENA_INFO* lpInfo) // OK
{
    BYTE send[256];
    int size = sizeof(PMSG_CUSTOM_ARENA_ENTRY) + 7; // C1+size+EE+count(4)+entry

    PMSG_CUSTOM_ARENA_ENTRY* pEntry = (PMSG_CUSTOM_ARENA_ENTRY*)(send+7);
    pEntry->Index = lpInfo->Index;
    pEntry->State = lpInfo->State;
    pEntry->RemainTime = lpInfo->RemainTime;
    pEntry->UserCount = this->GetUserCount(lpInfo);
    ...
    send[1] = size;
    send[2] = 0xEE;
    *(int*)(send+3) = 1; // count = 1 (single arena update)

    this->DataSendToAll(lpInfo, send, size);
}

void CCustomArena::GCSendScoreboard(CUSTOM_ARENA_INFO* lpInfo) // OK
{
    BYTE send[8192];
    int count = this->GetUserCount(lpInfo);
    int ofs = 11; // after C1+size+EF+arenaIndex(4)+count(4)

    for(int n=0; n<MAX_CUSTOM_ARENA_USER; n++)
    {
        if(OBJECT_RANGE(lpInfo->User[n].Index) == 0) continue;

        PMSG_CUSTOM_ARENA_SCORE_ENTRY* pEntry = (PMSG_CUSTOM_ARENA_SCORE_ENTRY*)&send[ofs];
        pEntry->Rank = lpInfo->User[n].Rank;
        memcpy(pEntry->Name, gObj[lpInfo->User[n].Index].Name, 10);
        pEntry->KillCount = lpInfo->User[n].UserKillCount;
        pEntry->DeathCount = lpInfo->User[n].UserDeathCount;
        pEntry->Score = lpInfo->User[n].Score;
    ...
    send[1] = ofs;
    send[2] = 0xEF;
    *(int*)(send+3) = lpInfo->Index;
    *(int*)(send+7) = count;

    this->DataSendToAll(lpInfo, send, ofs);
}
